...
parser.add_argument("--experiment_run_name",
...
train_dataset = GPTDatasetV1(txt=train_data, tokenizer=tokenizer,
                             max_length=llm_config["context_length"], stride=train_params["dataset"]["stride"],
                             annotate="Train dataset"
                             )
val_dataset = GPTDatasetV1(txt=val_data, tokenizer=tokenizer,
                             max_length=llm_config["context_length"], stride=train_params["dataset"]["stride"],
...
train_dataloader = create_dataloader_v1(dataset=train_dataset, batch_size=train_params["dataset"]["batch_size"],
                                        shuffle=True,
                                        drop_last=True,
                                        num_workers=0)
val_dataloader = create_dataloader_v1(dataset=val_dataset, batch_size=train_params["dataset"]["batch_size"],
                                        shuffle=False,
                                        drop_last=False,
                                        num_workers=0)
...
## set device
compute_device = torch.device("cpu" if torch.cuda.is_available() else "cpu")
...
with mlflow.start_run(run_name=experiment_run_name):
    ...
    for epoch in range(epochs):
        ...
        # Validation metrics
        val_loss = evaluate_model(model=model, val_dataloader=val_dataloader, compute_device=compute_device)
        val_perplexity = calculate_perplexity(model=model, dataloader=val_dataloader, compute_device=compute_device,
    ...
    for input_batch, target_batch in val_dataloader:
        with torch.no_grad():
            ...
            signature = mlflow.models.infer_signature(model_input=input_batch.numpy(), model_output=model_output.numpy())
        break
