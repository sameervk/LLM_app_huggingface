...
class MultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.W_query = torch.nn.Linear(in_features=embed_dim, out_features=self.total_dim_out, bias=qkv_bias)
        self.W_key = torch.nn.Linear(in_features=embed_dim, out_features=self.total_dim_out, bias=qkv_bias)
        self.W_value = torch.nn.Linear(in_features=embed_dim, out_features=self.total_dim_out, bias=qkv_bias)

        # dropout layer
        self.dropout = torch.nn.Dropout(p=dropout_p)

        # mask
        self.register_buffer(name='mask', tensor=torch.triu(torch.ones(size=(context_len, context_len)), diagonal=1))
    ...
    def forward(self, X):
        ...
        # reshape
        queries = queries.view(batch_size, num_tokens, self.num_heads, self.head_dim_out)
        ...
        # reshape
        context_vec = context_vec.contiguous().view(batch_size, num_tokens, self.total_dim_out)
...
if __name__=="__main__":
    ...
    multihead_attention = MultiHeadAttention(embed_dim=embed_dim,
