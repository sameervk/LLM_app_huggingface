...
class MaskedAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        self.W_query = torch.nn.Linear(
            in_features=embd_dim_in, out_features=dim_out, bias=qkv_bias
        )
        ...
        self.register_buffer(
            name="mask",
            tensor=torch.triu(
                input=torch.ones(size=(context_length, context_length)), diagonal=1
            ),
        )
    ...
    def forward(self, batch_inputs: torch.Tensor):
        ...
        # masking in-place
        batch_attn_scores.masked_fill_(
            self.mask.bool()[:num_tokens, :num_tokens], -torch.inf
        )
        # num_tokens can be less than context_length
        # context_length is the cut-off

        batch_attn_weights = torch.softmax(
            batch_attn_scores / batch_keys.shape[-1] ** 0.5, dim=-1
        )
...
if __name__ == "__main__":
    ...
    p_dropout = 0.5
    test_attention_layer = MaskedAttention(
        embd_dim_in=embed_dim, dim_out=5, context_length=cntxt_len, dropout=p_dropout
    )
